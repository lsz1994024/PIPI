...
public class PreSearch implements Callable<PreSearch.Entry> {
    ...
    public PreSearch(int scanNum, BuildIndex buildIndex, MassTool massTool, double ms2Tolerance, double ms1Tolerance, double leftInverseMs1Tolerance, double rightInverseMs1Tolerance
            ...
            , SpecProcessor specProcessor, InferPTM inferPTM, String truth) {
        ...
        for (ExpTag tagInfo : allLongTagList.subList(0, Math.min(2, allLongTagList.size()))){
            ...
            if (tagInfo.isNorC == N_TAG) { //n tag
                if (!searchedTagStrSet.contains(tagStr)) {
                    ...
                    int n_res = searchAndSaveFuzzy(scanNum ,tagInfo, ms1TolAbs, resPeptideListMap, peptideInfoMap, protIdSetByThisTag, fmIndex, tagChar, minTagLen, expProcessedPL, true);
                    ...
                    if (tagStr.length() > minTagLen){ // if the tag was already long i.e. there is space to sub
                        for (int n_cTermAaToCut = 1; n_cTermAaToCut <= Math.min(2,tagChar.length-minTagLen); n_cTermAaToCut++){
                            ...
                            if (!searchedTagStrSet.contains(tagStr)) {
                                int numResSub = searchAndSaveFuzzy(scanNum ,subTagInfo, ms1TolAbs, resPeptideListMap, peptideInfoMap, protIdSetByThisTag, fmIndex, subTagChar, minTagLen, expProcessedPL, false);
                                searchedTagStrSet.add(tagStr);
                                if (numResSub > 50) {
                                    break;
                                }
                ...
                if (!searchedTagStrSet.contains(revTagStr)) {
                    Set<String> protIdSetByThisTag = new HashSet<>();
                    int n_res = searchAndSaveFuzzy(scanNum ,revTagInfo, ms1TolAbs, resPeptideListMap, peptideInfoMap, protIdSetByThisTag, fmIndex, revTagChar, minTagLen, expProcessedPL, true);
                    ...
                    if (revTagStr.length() > minTagLen){ // if the tag was already long i.e. there is space to sub
                        for (int n_cTermAaToCut = 1; n_cTermAaToCut <= Math.min(2,revTagChar.length-minTagLen); n_cTermAaToCut++){
                            ...
                            if (!searchedTagStrSet.contains(subRevTagStr)) {
                                int numResSub = searchAndSaveFuzzy(scanNum ,subRevTagInfo, ms1TolAbs, resPeptideListMap, peptideInfoMap, protIdSetByThisTag, fmIndex, subRevTagChar, minTagLen, expProcessedPL, false);
                    ...
                    if (tagStr.length() > minTagLen){ // if the tag was already long i.e. there is space to sub
                        for (int n_cTermAaToCut = 1; n_cTermAaToCut <= Math.min(2,tagChar.length-minTagLen); n_cTermAaToCut++){
                            ...
                            if (!searchedTagStrSet.contains(subTagStr)) {
                                numResSub1 = searchAndSaveFuzzy(scanNum ,subTagInfo, ms1TolAbs, resPeptideListMap, peptideInfoMap, protIdSetByThisTag, fmIndex, subTagChar, minTagLen, expProcessedPL,false);
...
//    }
    ...
    private int searchAndSaveFuzzy(int scanNum, ExpTag tagInfo, double ms1TolAbs, Map<String, List<Peptide>> resPeptideListMap, Map<String, PeptideInfo> peptideInfoMap
            , Set<String> protIdSetByThisTag, FMIndex fmIndex, char[] tagChar, int minTagLen, SparseVector expProcessedPL, boolean isFirstTime) throws GRBException {
        ...
        if (searchRes.settled) {
            for (int ii = searchRes.sp; ii <= searchRes.ep; ii++) {
                ...
                updateCandiList(scanNum, protId, relPos, tagInfo, ms1TolAbs, resPeptideListMap, peptideInfoMap, expProcessedPL);
            }
        } else {
            ...
            for (int ii = searchRes.sp; ii <= searchRes.ep; ii++) {
                ...
                int relPos = absTagPos - buildIndex.dotPosArrReduced[dotIndex] - 1;
                updateCandiList(scanNum, protId, relPos, tagInfo.subTag(matchedPos, tagChar.length), ms1TolAbs, resPeptideListMap, peptideInfoMap, expProcessedPL);
            }
...
//        System.out.println(scanNum + "," + tagInfo.getFreeAaString() + "," + numRes);
    ...
    private void updateCandiList(int scanNum, String protId, int tagPosInProt, ExpTag finderTag, double ms1TolAbs, Map<String, List<Peptide>> resPeptideListMap, Map<String, PeptideInfo> peptideInfoMap, SparseVector expProcessedPL) throws GRBException {
        ...
        if (finderTag.isNorC == C_TAG || Math.abs(tagCMass - precursorMass) < 0.02){ // C tag, the end of tag must be KR and the tagCMass must be exactly settle. Otherwise no valid cPos will be, then no valid n pos.
            if (isKR(protSeq.charAt(tagPosInProt+finderTag.size()-1)) && Math.abs(tagCMass - precursorMass) < 0.02) { //fixme, should use MS1 tolerance not MS2 tolerance
        ...
        for (int cPos : cPoscMassMap.keySet()) {
            ...
            if (Math.abs(cDeltaMass) > 0.1) { // not ptm free in c part
                cPosVarPtmResMap = inferPTM.settleSideWithPtm(scanNum, cPartSeq, cDeltaMass, C_PART, isProtNorC_Term, cCutMass, expProcessedPL,ms1TolAbs);
            ...
            for (int nPos = max_nPos; nPos >= min_nPos; nPos--) {
                ...
                if (Math.abs(nDeltaMass) > 0.1){
                    nPosVarPtmResMap = inferPTM.settleSideWithPtm(scanNum, nPartSeq, nDeltaMass, N_PART, isProtNorC_Term, nCutMass, expProcessedPL, ms1TolAbs);
...
//        System.out.println(scanNum + ", n_extention," + finderTag.getFreeAaString() +","+ n_extension);
    ...
    public class PresearchEntry {
        ...
        public List<Peptide> resPeptideListMap = new ArrayList<>();
