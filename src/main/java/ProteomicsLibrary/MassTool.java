...
//    public Map<Character, VarPtm> labelVarPtmMap = new HashMap<>();
    ...
    public Set<String> dummyDigest(String proteinSequence, int missCleav) {
        Set<String> peptideSeqSet = new HashSet<>();

        Map<Integer, List<int[]>> digestRangeMap1 = digest(proteinSequence, digestSitePattern1, cleavageFromCTerm1, missCleav);
        for (int i : digestRangeMap1.keySet()) {
            for (int[] digestRange : digestRangeMap1.get(i)) {
                String subString = proteinSequence.substring(digestRange[0], digestRange[1]);
                peptideSeqSet.add("n" + subString + "c");
        ...
        if (digestSitePattern2 != null) {
            Map<Integer, List<int[]>> digestRangeMap2 = digest(proteinSequence, digestSitePattern2, cleavageFromCTerm2, missCleav);
            for (int i : digestRangeMap2.keySet()) {
                for (int[] digestRange : digestRangeMap2.get(i)) {
                    String subString = proteinSequence.substring(digestRange[0], digestRange[1]);
                    peptideSeqSet.add("n" + subString + "c");
        ...
        if (proteinSequence.startsWith("M")) {
            String newSequence = proteinSequence.substring(1);
            digestRangeMap1 = digest(newSequence, digestSitePattern1, cleavageFromCTerm1, missCleav);
            for (int i : digestRangeMap1.keySet()) {
                int[] digestRange = digestRangeMap1.get(i).get(0);
                String subString = newSequence.substring(digestRange[0], digestRange[1]);
                peptideSeqSet.add("n" + subString + "c");
            ...
            if (digestSitePattern2 != null) {
                Map<Integer, List<int[]>> digestRangeMap2 = digest(newSequence, digestSitePattern2, cleavageFromCTerm2, missCleav);
                for (int i : digestRangeMap2.keySet()) {
                    int[] digestRange = digestRangeMap2.get(i).get(0);
                    String subString = newSequence.substring(digestRange[0], digestRange[1]);
                    peptideSeqSet.add("n" + subString + "c");
    ...
    public double[][] buildTheoBYIonsArray(String sequence, int maxCharge) { // there are n and c in the sequence
        ...
        double[][] peptideIonArray = new double[2 * maxCharge][aaArray.length - 1];
        peptideIonArray[0][0] = PROTON;
        peptideIonArray[1][aaArray.length - 2] = H2O + PROTON;
        ...
        for (int i = 1; i < aaArray.length - 2; ++i) {
            ...
            for (int charge = 1; charge <= maxCharge; ++charge) {
                peptideIonArray[2 * (charge - 1)][i]  = bIonMass + PROTON;
            }
        }
        ...
        for (int charge = 1; charge <= maxCharge; ++charge) {
            peptideIonArray[2 * (charge - 1)][aaArray.length - 2] = bIonMass + PROTON;
        }
        ...
        for (int charge = 1; charge <= maxCharge; ++charge) {
            peptideIonArray[2 * (charge - 1) + 1][0] = yIonMass + PROTON;
        }
        ...
        for (int charge = 1; charge <= maxCharge; ++charge) {
            peptideIonArray[2 * (charge - 1) + 1][1] = yIonMass + PROTON;
        }
        ...
        for (int i = 2; i < aaArray.length - 2; ++i) {
            yIonMass -= massTable.get(aaArray[i].aa) + aaArray[i].ptmDeltaMass;
            for (int charge = 1; charge <= maxCharge; ++charge) {
                peptideIonArray[2 * (charge - 1) + 1][i] = yIonMass + PROTON;
            }
        }
...
//        System.out.println("=========================");
    ...
    public double buildVectorAndCalXCorr(double[][] ionMatrix, int precursorCharge, SparseVector xcorrPL, List<Integer> jRange) {
        int colNum = ionMatrix[0].length;
        int rowNum = Math.min(ionMatrix.length / 2, precursorCharge - 1) * 2;
        if (precursorCharge == 1) {
            rowNum = 2;
...
//                    System.out.println(i+","+j+","+ionMatrix[i][j]+  ","+ xcorrPL.get(mzToBin(ionMatrix[i][j])));
    ...
    public double buildVectorAndCalXCorr(double[][] ionMatrix, int precursorCharge, SparseVector xcorrPL, Map<Integer, Double> matchedBions, Map<Integer, Double> matchedYions,  Set<Integer> jRangeB, Set<Integer> jRangeY) {
        ...
        double xcorr = 0;
        for (int j : jRangeB) {
            double peakIntensity = xcorrPL.get(mzToBin(ionMatrix[0][j]));
            if (peakIntensity > 0.0) {
                matchedBions.put(j+1, peakIntensity);
            }
            xcorr += xcorrPL.get(mzToBin(ionMatrix[0][j]));
        }
        for (int j : jRangeY) {
            double peakIntensity = xcorrPL.get(mzToBin(ionMatrix[1][j]));
            if (peakIntensity > 0.0) {
                matchedYions.put(j+1, peakIntensity);
            }
            xcorr += xcorrPL.get(mzToBin(ionMatrix[1][j]));
        }
        return xcorr * 0.25;
    }
